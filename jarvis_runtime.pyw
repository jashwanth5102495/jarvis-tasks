...
jarvis_runtime.pyw
===================
Main entry point for JARVIS AI Operating System persistent runtime (no terminal).
...
logging.basicConfig(
    ...
    handlers=[
        ...
        # No StreamHandler so no terminal output
    ]
)
...
def run_asyncio_runtime(loop):
    ...
    from runtime_core.persistent_runtime import persistent_runtime
...
def run_legacy_runtime_in_thread():
    ...
    from runtime_core.event_bus import event_bus, EventType, Event
    ...
    def on_ctrl_p():
        from runtime_core.runtime_state_machine import runtime_state_machine, RuntimeState
...
def main():
    logger.info("=" * 70)
    logger.info("JARVIS AI Operating System - Persistent Runtime Starting (No Terminal)")
    logger.info("=" * 70)

    # Import UI components
    from runtime_ui.presence_manager import presence_manager
    from runtime_ui.overlay_manager import overlay_manager
    from runtime_ui.tray_manager import tray_manager
